...
<script setup>
  ...
  const isShowingScore = ref(false)

  const apiKey = "d758b031-1a51-4e92-9598-86a30e5639c6"

  onMounted(() => {
    console.clear()
    AOS.init();

    searchDate.value = getCurrentDate ()
  })
  ...
  watch(searchDate,(newDate, oldDate) => {
    if (newDate !== oldDate) fetchGames();
  })

  const fetchGames = async () =>{
    isFetchingData.value = true;
    
    const URL = `https://api.balldontlie.io/v1/games?start_date=${searchDate.value}&end_date=${searchDate.value}`;
    const options = {
      headers: {
  ...
  //   let currentDate = moment(searchDate.value).tz('Asia/Tokyo');
  //   currentDate.add(days, 'days');
  //   searchDate.value = currentDate.format('YYYY-MM-DD');
  // })

  const getTeamLogoPosition = ((abbreviation) => {
  ...
  const showScore = (() => {
    if(!isShowingScore.value){
      isShowingScore.value = !isShowingScore.value
  ...
  const isGameHot= ((game) => {
    ...
    if (
      ...
      targets.includes(game.visitor_team.abbreviation)
    ) {
      return true
    }
    ...
    if(scoreDifference <= 8) return true
  ...
  const getBackgroundColor = ((abbreviation) => {
  ...
  const dateRange = computed(() => {
    const now = moment(searchDate.value).tz('Asia/Tokyo');
    let dates = [];
